...
class SuperHeroine(object):
    ...
    def __str__(self):
        ...
        *As a side note, often in object-oriented programming, __str__ methods
        ...
        description = f"{self.name} is a member of the {self.team}, and has the following powers:\n{self.powers}"
        return description 
...
def main():
    ...
    for item in basic_info:
        name = item["name"]
        string = SuperHeroine(item["name"], item["full_name"], item["team"], item["eye_color"], item["hair_color"], item["base"])
        heroines[name] = string
    ...
    #       save that instance to a new variable <instance_affected> 
        instance_affected = heroines[name_value]
        #print(instance_affected)
    ...
    #       is 'power' or 'nemesis'.
        if category_value == "power":
            instance_affected.add_power(value_value)
        else:
            instance_affected.add_nemesis(value_value)

        heroines["Heroine Name"] = instance_affected
