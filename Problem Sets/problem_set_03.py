...
def filter_by_units_sold(console,minimum_units_sold):
    unit = console.split(',')[-1]
    unit = int(unit)
    #print(unit)
    if unit>minimum_units_sold:
        return True
    else:
        return False
    """
...
def format_string(console):
    name = console.split(',')[0]
    year = console.split(',')[-2]
    company = console.split(',')[1]
...
def main(console_list):
    ...
    for console in console_list:
        response = filter_by_units_sold(console,100000000 )
        if response == True:
           out = format_string(console)
           success.append(out)
    return success
