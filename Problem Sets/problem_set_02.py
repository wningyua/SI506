...
def umich_email_detection(potential_email):
    ...
    """
    x = potential_email.endswith("@umich.edu")
    if x == True:
        out=potential_email.replace("@umich.edu","")
    else:
        out=""
    return f"{out}"
...
def count_words_or_lines(string,operation="words"):
    if operation=='words':
        ...
        x=len(string.splitlines())
        return x
    else:
        x="Choose a correct operation"
        return f"{x}"
...
num_words = count_words_or_lines(string=mission)
