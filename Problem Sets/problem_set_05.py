...
def create_step_length(step_list):
    ...
    for step in step_list:
        ...
        nums.append(num)
        orders.append(order)
    dictionary = dict(zip(orders, nums))
    return dictionary
...
def filter_step_length(step_length):
    ...
    max_length = 0
    # find cutoff
    for val in step_length.values():
        if val > max_length:
            max_length = val
        cutoff = max_length/2

    for key, val in step_length.items():
        if val >=cutoff:
...
# Hint: You can use enumerate() to help generate the order. Notice that order should start at 1.
...
def main(step_list):
    ...
    """
    # step_new = []
    # enum_step = enumerate(step_list)
    # for tup in enum_step:
    #     order = int(tup[0] + 1)
    #     f_tup = format_step(tup[1], order)
    #     step_new.append(f_tup)
    # #print(step_new)
    # core_step_and_length = create_step_length(step_new)
    # #print(core_step_and_length)
    # results  = filter_step_length(core_step_and_length)
    # results = list(results.keys())
    # print(results)
    # new_results = []
    # for result in results:
    #     result = result.replace("step", "")
